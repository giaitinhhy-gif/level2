 ...
 */
const std::string VERSION = "0.4.<25022800>";
...
void showSelectMenu(SelectType selectType)
{
    ...
    case SelectType::SIZE_UI:
        std::cout << std::format("Board Size (NxN, 3 <= N <= {}): ", BOARD_N_MAX);
        break;

    case SelectType::GOAL_UI:
        std::cout << "Win Goal (consecutive symbols, 3 <= goal <= size): ";
        break;

    case SelectType::GAME_MODE_UI:
        std::cout << "Game Mode: (1) PvP  (2) PvE  (3) EvE\nYour choice: ";
        break;
...
void showInvalidMove()
{
    std::cout << "[!] Invalid move! Cell is occupied or out of bounds. Try again.\n";
}
               ...
               GameSetup &gameSetup)
{
    ...
    {
        std::cout << "Select bot level for Player 2:\n";
        while (true)
        {
    ...
    {
        std::cout << "Select bot level for Bot 1:\n";
        while (true)
        {
        ...
        std::cout << "Select bot level for Bot 2:\n";
        while (true)
        {
            showSelectMenu(SelectType::MUL_BOT_LEVEL_UI);
            if (selectBotLevel(gameSetup.levels, 1))
    ...
    while (true)
    {
        ...
        else
        {
            ...
            while (!validInput)
            {
                ...
                {
                    if (std::cin.eof()) // ← thêm dòng này
                        return result;  // ← thoát khi hết input
                    showInvalidMove();
                }
            }
        }

        makeMove(gameSetup.board, row, col, sym);
        result.turns++;

        if (checkWin(gameSetup.board, gameSetup.size, sym, gameSetup.goal, EndRule::OPEN_TWO))
        {
             ...
             GameResult &gameResult)
{
    ...
    GameLogger::log(std::format("Game ended. Turns: {}, Winner: {}", gameResult.turns, gameResult.winner));
}
                     ...
                     const char playerSymbol)
{
    for (int i = 0; i < size; i++)
    {
        for (int j = 0; j < size; j++)
        {
            if (board[i][j] != '-')
                continue;
            board[i][j] = botSymbol;
            bool wins = checkWin(board, size, botSymbol, goal, EndRule::NONE);
            board[i][j] = '-';
            if (wins)
                return {i, j};
        }
    ...
    for (int i = 0; i < size; i++)
    {
        for (int j = 0; j < size; j++)
        {
            ...
            bool wins = checkWin(board, size, playerSymbol, goal, EndRule::NONE);
            board[i][j] = '-';
            if (wins)
                return {i, j};
        }
    }
    // 3. Prefer center, then near existing pieces
    int center = size / 2;
