...
bool SDLInteraction::selectSize(int* size) {
    ...
    while (SDL_WaitEvent(&e)) {
        ...
        if (e.type == SDL_KEYDOWN) {
            ...
            } else if (e.key.keysym.sym == SDLK_RETURN && !input.empty()) {
                *size = std::stoi(input);
...
bool SDLInteraction::selectGameMode(GameMode* mode) {
    ...
    while (SDL_WaitEvent(&e)) {
        waitForQuit(e);
        if (e.type == SDL_KEYDOWN) {
            if (e.key.keysym.sym == SDLK_1) { *mode = GameMode::PVP; return true; }
            if (e.key.keysym.sym == SDLK_2) { *mode = GameMode::PVE; return true; }
            if (e.key.keysym.sym == SDLK_3) { *mode = GameMode::EVE; return true; }
        }
    }
    //throw NotImplementedException();
    return false;
...
bool SDLInteraction::selectBotLevel(BotLevel* levels, const int index) {
    ...
    while (SDL_WaitEvent(&e)) {
        waitForQuit(e);
        if (e.type == SDL_KEYDOWN) {
            if (e.key.keysym.sym == SDLK_1) { levels[index] = BotLevel::EASY;   return true; }
            if (e.key.keysym.sym == SDLK_2) { levels[index] = BotLevel::MEDIUM; return true; }
            if (e.key.keysym.sym == SDLK_3) { levels[index] = BotLevel::HARD;   return true; }
        }
    }
    //throw NotImplementedException();
    return false;
...
bool SDLInteraction::getPlayerMove(int* row, int* col) {
    ...
    while (SDL_WaitEvent(&e)) {
        ...
        if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT) {
            int padding  = 50;
            int cellSize = (800 - 2 * padding) / boardSize;
            *row = (e.button.y - padding) / cellSize;
            *col = (e.button.x - padding) / cellSize;
            return true;
        }
    }
    //throw NotImplementedException();
