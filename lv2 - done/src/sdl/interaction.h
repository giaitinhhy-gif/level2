...
struct ClickArea {
    SDL_Rect rect;   // vùng click (x, y, w, h)
    int      value;  // giá trị trả về khi click vào vùng này
};
...
class SDLInteraction : public I_Interaction {
   private:
    SDLRenderer* sdlRenderer = nullptr;
    int boardSize = 3;
    /**
    ...
    int waitForClick(const std::vector<ClickArea>& areas);
   ...
   public:
    ...
    void setRenderer(SDLRenderer* r) { sdlRenderer = r; }
    ...
    int waitForPicker(int initVal, int minVal, int maxVal,
                  SelectType menuType, int context = NO_CONTEXT);
